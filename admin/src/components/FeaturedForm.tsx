...
export default function FeaturedForm({ project, onSuccess, onCancel }: FeaturedFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = project?.id ? `${API_BASE_URL}/api/featured/${project.id}` : `${API_BASE_URL}/api/featured`;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4 pt-4">
                ...
                >
                    {loading ? 'Saving...' : project?.id ? 'Update Featured' : 'Create Featured'}
                </button>
            </div>
        </form>
    );
